...
export const rowSeatSlice = createSlice({
  name: "booking",
  initialState,
  reducers: {
    bookSeat: (state, actions) => {
      const { soGhe } = actions.payload;
      state.seats = state.seats.map((row) => ({
        ...row,
        danhSachGhe: row.danhSachGhe.map((seat) =>
          seat.soGhe === soGhe ? { ...seat, daDat: true } : seat
        ),
      }));
    },
  },
...
export const { bookSeat } = rowSeatSlice.actions;

export default rowSeatSlice.reducer;
